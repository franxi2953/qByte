...
- [Making the device](#making-the-device)
- [Results](#results)
...
4. Set the serial monitor to **115200 baud**. If the output is unreadable, verify the selected port and baud rate.

5. Configure the device's Wi-Fi by sending `wifi "ssid" "password"` (including the quotes) in the serial monitor. The device will restart and connect to that network. The network only needs internet access when checking for firmware updates.

6. Connect to the same Wi-Fi network on your computer or mobile device.

7. Open [http://qlamp.local](http://qlamp.local) in a browser. To change the local hostname, send `mDNS "new-name"` without the `.local` suffix. Send `mDNS` to display the current name.
...
The supported eight-well firmware is in [`Production/embedded system/code/main`](Production/embedded%20system/code/main). The separate [`Production/embedded system - 2 channel`](Production/embedded%20system%20-%202%20channel) directory is an experimental two-channel hardware variant.

- The ESP32 stores its web interface, configuration, protocols, and experiment data in [SPIFFS](https://randomnerdtutorials.com/install-esp32-filesystem-uploader-arduino-ide/).
- The device serves the interface at `http://<device-name>.local/` and at its assigned IP address.
- Wi-Fi and mDNS settings can be changed from the web interface or serial console.
- Firmware and interface updates can be started from the web interface or with the serial command `update`. Updates are published from this GitHub repository; see the [firmware update documentation](Production/embedded%20system/update_server/README.md).
- The HTTP control endpoints are documented in [`API_DOCUMENTATION.md`](Production/embedded%20system/API_DOCUMENTATION.md).
